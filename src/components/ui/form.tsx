...
const FormControl = React.forwardRef<React.ElementRef<typeof Slot>, React.ComponentPropsWithoutRef<typeof Slot>>(({ ...props }, ref) => {
  ...
  return (
    <Slot
      ...
      aria-invalid={!!error}
      className={cn(error && 'border-destructive/50 focus-visible:ring-destructive')}
      {...props}
    />
  );
});
...
const FormDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(({ className, ...props }, ref) => {
  ...
  return <p ref={ref} id={formDescriptionId} className={cn('absolute -bottom-5 text-[0.8rem] text-muted-foreground', className)} {...props} />;
});
...
const FormMessage = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(({ className, children, ...props }, ref) => {
  ...
  return (
    <p ref={ref} id={formMessageId} className={cn('-bottom-4.5 absolute right-0 text-xs font-medium text-destructive', className)} {...props}>
      {body}
    </p>
  );
});
